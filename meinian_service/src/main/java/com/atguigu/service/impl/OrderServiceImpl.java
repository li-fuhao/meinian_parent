...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public Map findById4Detail(Integer orderId) throws Exception {
    ...
    public Result saveOrder(Map map) throws Exception {
        ...
        Member member = memberDao.getMemberByTelephone(telephone);//数据库会员
        if (member == null){//不存在就快速注册
            ...
            member.setPhoneNumber(telephone);//上面65行，取过了
            member.setRegTime(new Date());
            memberDao.add(member);//添加新的会员，主键回填，不回填，member没有主键
        }else {//检查是否重复预约
            //将findOrderByCondition()方法封装为一个通用方法，可以实现动态SQL查询
